...
void buildHeader()
{
    ...
    int totalAtt=3; // # of attributes (it is used to fill the header - last line before fclose)
    /// first field (attribute id string 5)
    strcpy(fname,"id");
    ftype='S';
    flen=5;
    fwrite(fname,15,1,f);
    ...
    //***fwrite(&ftype,1,1,f);
    //***fwrite(&flen,sizeof(int),1,f);
    /// Our head supports 10 (MFIELD) attributes
    /// We use 4, we have to fill the left space with #
    strcpy(fname,"#");
    fwrite(fname,15+1+sizeof(int),MFIELD-totalAtt,f);
    fclose(f);
}
